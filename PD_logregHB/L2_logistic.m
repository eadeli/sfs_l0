function [x, fval, time] = L2_logistic(A,rho,z,lambda,x0,tol,M,maxiter)
...
[Fx,eAx] = Fval_L1(A,x);
Fx = Fx/m+norm(x-z)^2*rho/2+norm(x)^2*(lambda/2);
gx = A(1-1./eAx,2)/m+x*lambda;
...
while iter <= maxiter && err > tol
    while 1 == 1
        ...
        Fxnew = Fxnew/m+norm(xnew-z)^2*rho/2+norm(xnew)^2*(lambda/2);
        nf = nf + 1;
    ...
    gxnew = A(1-1./eAx,2)/m+xnew*lambda;
    dx = xnew - x; 
    dg = gxnew - gx;
