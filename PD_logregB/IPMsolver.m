...
function [x,d] = IPMsolver(A,mu,sigma,b,c,rho,x,d)
global typeA Afull
eps = 1e-4;
[n,p] = size(A);
iter = 1; 
...
while (1)
    [g,D0] = grad_hess(A,mu,sigma,b,c,rho,x,expf);
    err = norm(g,'inf');
    if err < eps 
        break;
    end
    ...
    delta = g'*d;
    [x,f,expf] = linesearch(A,mu,sigma,b,c,rho,x,d,delta,f);
    iter = iter + 1;
end

     
function [g,D0] = grad_hess(A,mu,sigma,b,c,rho,x,expz)
n = size(A,1);
tmp = 1/n./(ones(n,1)+1./expz);
g = -operator(A,mu,sigma,b,tmp,2) + rho*(x-c); 
D0 = 1/n./(2*ones(n,1) + expz + 1./expz);


function [x_new,f_new,expf] = linesearch(A,mu,sigma,b,c,rho,x,d,delta,f)
alpha = 0.01;
beta = 0.5;
s = 1;
while (1)
